...
for top_replay in top_50_replays:
    ...
    if comparison[1] >= 95:
        suspicious = True
        print("Suspicious same keys pressed percentage: {0:.2f}% with {top_player}'s replay".format(comparison[1],
    ...
    if suspicious_low_values:
        suspicious = True
        print("Suspicious lowest values with {top_player}'s replay".format(top_player=top_replay[2]))
    print_chan("\nAverage of similarity:")
    average_value = sum(comparison[0]) / len(comparison[0])
    if average_value <= 15:
        suspicious = True
        print("Suspicious average of similarity: {0:.4f} with {top_player}'s replay".format(average_value,
              top_player=top_replay[2]))
